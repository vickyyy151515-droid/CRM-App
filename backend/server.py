from fastapi import FastAPI, APIRouter, HTTPException, UploadFile, File, Depends, status, Form, Request, Query
...
# App initialization
app = FastAPI(title="CRM Pro API", version="2.1.0")  # Updated version for refactored codebase
...
# ==================== LEAVE REQUEST (OFF DAY/SAKIT) ENDPOINTS ====================

MONTHLY_LEAVE_HOURS = 24  # 24 hours of paid leave per month
OFF_DAY_HOURS = 12  # Off day deducts 12 hours

class LeaveRequestCreate(BaseModel):
    leave_type: str  # 'off_day' or 'sakit'
    date: str  # YYYY-MM-DD
    start_time: Optional[str] = None  # HH:MM (for sakit)
    end_time: Optional[str] = None  # HH:MM (for sakit)
...
def calculate_leave_hours(leave_type: str, start_time: str = None, end_time: str = None) -> float:
    """Calculate hours to deduct based on leave type"""
...
async def get_leave_balance(
    ...
    """Get remaining leave balance for current user"""
    if year is None:
        year = get_jakarta_now().year
    if month is None:
        month = get_jakarta_now().month
...
async def create_leave_request(
    ...
    if request_data.leave_type not in ['off_day', 'sakit']:
        raise HTTPException(status_code=400, detail="Invalid leave type. Use 'off_day' or 'sakit'")
    
    # Validate times for sakit
    if request_data.leave_type == 'sakit':
        if not request_data.start_time or not request_data.end_time:
            raise HTTPException(status_code=400, detail="Start time and end time are required for sick leave")
    
    # Calculate hours to deduct
    hours_deducted = calculate_leave_hours(
    ...
    # Include pending requests in calculation
    pending_query = {
        'staff_id': user.id,
        'date': {'$regex': f'^{year}-{str(month).zfill(2)}'},
        'status': 'pending'
    }
    ...
    if hours_deducted > available_hours:
        raise HTTPException(
            status_code=400, 
            detail=f"Insufficient leave balance. Available: {available_hours} hours, Requested: {hours_deducted} hours"
        )
...
# Admin endpoints for leave requests
@api_router.get("/leave/all-requests")
...
@api_router.get("/leave/staff-balance/{staff_id}")
async def get_staff_leave_balance(
    ...
    """Get leave balance for a specific staff (admin only)"""
    if year is None:
        year = get_jakarta_now().year
    if month is None:
        month = get_jakarta_now().month
...
@api_router.put("/leave/request/{request_id}/action")
