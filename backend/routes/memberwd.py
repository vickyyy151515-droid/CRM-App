...
async def upload_memberwd_database(
    ...
    for idx, row in df.iterrows():
        ...
        # Check if this record is a reserved member using centralized utility
        ...
        record = {
            ...
            'row_number': idx + 1,
            'row_data': row_data,
            'status': 'reserved' if is_reserved_flag else 'available',
            'assigned_to': None,
...
async def assign_memberwd_records(assignment: MemberWDAssignment, user: User = Depends(get_admin_user)):
    ...
    if not batch_id:
        ...
        first_record = await db.memberwd_records.find_one({'id': allowed_ids[0]}, {'_id': 0})
        ...
        await db.memberwd_batches.update_one(
            {'id': batch_id},
            {'$inc': {'current_count': len(allowed_ids)}}
        )
...
@router.get("/memberwd/staff/invalidated-by-reservation")
async def get_staff_memberwd_invalidated_by_reservation(
    ...
    records = await db.memberwd_records.find(query, {'_id': 0}).sort('invalidated_at', -1).to_list(1000)
...
async def diagnose_memberwd_product_mismatch(user: User = Depends(get_admin_user)):
    """
    Preview what the product mismatch repair would fix WITHOUT making changes.
    Uses shared diagnosis utility.
    """
    from utils.repair_helpers import diagnose_product_mismatch
    
    db = get_db()
    return await diagnose_product_mismatch(db, module='memberwd')


@router.post("/memberwd/admin/repair-product-mismatch")
...
async def diagnose_memberwd_reserved_conflicts(user: User = Depends(get_admin_user)):
    ...
    Uses shared diagnosis utility.
    """
    from utils.repair_helpers import diagnose_reserved_conflicts
    
    db = get_db()
    return await diagnose_reserved_conflicts(db, module='memberwd')
...
async def fix_memberwd_reserved_conflicts(user: User = Depends(get_admin_user)):
    ...
    """
    from utils.repair_helpers import fix_reserved_conflicts
    
    db = get_db()
    return await fix_reserved_conflicts(db, module='memberwd')
...
async def get_memberwd_data_health(user: User = Depends(get_admin_user)):
    ...
    Uses shared health check utilities + MemberWD-specific batch health check.
    """
    from utils.repair_helpers import run_full_health_check, check_batch_health
    
    db = get_db()
    
    # Run standard health check
    health_report = await run_full_health_check(db, module='memberwd')
    
    # Add MemberWD-specific batch health check
    batch_health = await check_batch_health(db)
    health_report['batches'] = batch_health['batches']
    health_report['batch_mismatches'] = batch_health['batch_mismatches']
    health_report['total_issues'] += batch_health['batch_mismatches']
