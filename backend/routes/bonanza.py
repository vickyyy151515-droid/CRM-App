...
from utils.reserved_check import build_reserved_set, build_reserved_map, is_record_reserved, find_reservation_owner, ensure_reserved_status_for_database
...
async def get_bonanza_databases(product_id: Optional[str] = None, user: User = Depends(get_admin_user)):
    ...
    for database in databases:
        ...
        reserved = await db.bonanza_records.count_documents({'database_id': database['id'], 'status': 'reserved'})
...
async def repair_bonanza_data(user: User = Depends(get_admin_user)):
    ...
    Uses shared repair utilities for consistency across modules.
    """
    from utils.repair_helpers import run_full_repair
...
async def repair_product_mismatch(user: User = Depends(get_admin_user)):
    ...
    """
    from utils.repair_helpers import repair_product_mismatch as _repair
    
    db = get_db()
    return await _repair(db, module='bonanza')
...
async def diagnose_product_mismatch(user: User = Depends(get_admin_user)):
    ...
    """
    from utils.repair_helpers import diagnose_product_mismatch as _diagnose
